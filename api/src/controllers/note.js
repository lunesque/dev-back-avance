...
router.get(
  ...
  async (req, res) => {
    try {
      const note = await NoteObject.findOne({
        _id: req.params.id,
...
router.get(
  ...
  async (req, res) => {
    try {
      if (
        req.user._id.toString() !== req.params.user_id &&
        req.user.role !== "admin"
      )
        return res.status(403).send({ ok: false, code: "FORBIDDEN" });
...
router.delete(
  ...
  async (req, res) => {
    try {
      const result = await NoteObject.deleteOne({
        _id: req.params.id,
        user_id: req.user._id,
      });
      if (result.deletedCount === 0)
        res.status(403).send({ ok: false, code: "FORBIDDEN" });
